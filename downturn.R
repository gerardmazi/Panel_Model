...
wd = 'C:/Users/qew864/OneDrive - Capital One Financial Corporation/3. DOWNTURN'
code = paste0(wd, '/CODE')
...
def = read.csv(paste0(input,'/Upgrades_Downgrades (SnP Raw Data).csv'), header = F, skip = 10, nrows = 7)
def = def[,-1]
def = t(def)
...
def$date = as.Date(def$`Up/Down`, format = "%m/%d/%Y")

# Assign quarter
def$Quarter = as.yearqtr(def$date, "%m/%d/%Y")
def$Quarter = paste(substr(def$Quarter, 1, 4), ':', substr(def$Quarter, 6, 7), sep="")

# Format as numeric for further data processing
index = sapply(def, is.factor)
def[index] = lapply(def[index], function(x) as.numeric(as.character(x)))
...
sd = mean(def$def_rate, na.rm = TRUE) + sd(def$def_rate, na.rm = TRUE)
def$downturn.sd = as.numeric(def$def_rate > sd)

# 80th Percentile approach (def rate > 80th percentile def rate)
pct = as.double(quantile(def$def_rate, 0.8, na.rm = TRUE))
...
get_default_rate = function(data, rating) {
  ...
  data$Quarter = paste(substr(data$Quarter, 1, 4), ':', substr(data$Quarter, 6, 7), sep="")
  ...
  # Re-arrange the column order with quarter first
  data = cbind(date=data$date, Quarter=data$Quarter, data[,2:7])
...
for (rating in rating_list) {
  dt[[rating]] = read.csv(paste0(input,'/', rating, '.csv'), header = F, skip = 10, nrows = 7, stringsAsFactors = F)
...
rating_def = data.frame(Quarter = AAA$Quarter,
                    ...
                    BB = BB$BB_def_rate, 
                    B = B$B_def_rate, 
                    CCC_C = CCC_C$CCC_C_default_rate, stringsAsFactors = F)

#================ Get rating weighted default rates  ================#

# COF Portfolio Weights
weights = read.table(paste0(input, '/weight_201612.txt'), stringsAsFactors = F)
...
# Standard Deviation approach (def rate > 1 standard deviation above mean)
sd_w = mean(rating_def$average_def_rate, na.rm = TRUE) + sd(rating_def$average_def_rate, na.rm = TRUE)
def$downturn.sd_w = as.numeric(rating_def$average_def_rate > sd_w)

# 80th Percentile approach (def rate > 80th percentile def rate)
pct_w = as.double(quantile(rating_def$average_def_rate, 0.8, na.rm = TRUE))
def$downturn.pct_w = as.numeric(rating_def$average_def_rate > pct_w)

# Median approach (def rate > 2 x Median)
med_w = median(rating_def$average_def_rate, na.rm  = TRUE) * 2
def$downturn.med_w = as.numeric(rating_def$average_def_rate > med_w)

write.csv(def, file=paste0(output, '/def_out.csv'), row.names = F)
